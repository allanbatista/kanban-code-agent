...
| 1 | `CodexAgentClient` inproc: spawn `codex app-server`, handshake, thread/turn, `outputSchema` de decisão, eventos→chat (item/agentMessage) | `src/application/codex-client.ts` (novo), `src/cli/codex-runner.ts` (novo), testes com app-server fake (stdio) | Teste roda task fake ponta a ponta com decisão estruturada; interrupt/cancel via `turn/interrupt` | done — prompts reusam PiAgentClient por composição; RoutingAgentClient lazy na factory |
| 1 | MCP bridge das tools do Master + `config.toml` do CODEX_HOME | `src/worker/mcp-bridge.ts` (novo), `src/infrastructure/security/codex-home.ts` (novo) | `create_subtask`/`post_message` chamadas pelo Codex chegam ao Orquestrator em teste | done — MCP hand-rolled (~180 linhas, sem SDK); GET /tools no tool server; config.toml idempotente |
| 1 | Auth Codex compartilhada: CODEX_HOME em `.swarm/auth/codex/`, rotas login device-code/status/logout, UI em Settings | `src/infrastructure/api/routes/settings.ts`, `web/src/components/settings/ProvidersSettings.tsx` | Login device-code exibe URL+código na UI; `account/read` reporta logado; auth.json com 0600 | done — protocolo validado contra codex-cli 0.144.1 real; key só via stdin; login serializado (409) |
| 2 | `SWARM_ISOLATION=docker`: `buildDockerRunArgs`, tradução de paths host↔container, cancel via `docker stop`, limites mem/cpu | `src/infrastructure/process/worker-supervisor.ts`, `src/infrastructure/config.ts`, testes de args | Teste valida args docker (mounts, limites, rm); run real em container passa smoke `/health` via socket montado | done — translateToContainer testado; secrets via --env-file 0600, nunca argv |
| 2 | Worker bundle: esbuild single-file + estágio de build dos binários (node, codex) montáveis | `scripts/build-worker-bundle.mjs` (novo), `package.json` | Bundle roda `--help` num container `debian:bookworm-slim` sem node instalado | done — pi SDK bundlou limpo; codex é static-pie musl (roda sozinho em glibc) |
| 2 | Remover systemd (código, envvars, docs) | `worker-supervisor.ts`, `config.ts`, `docs/`, testes | `rg -i systemd src/` vazio; suíte verde | done — verificado pelo manager: grep vazio |
| 2 | Worker multi-agent: `worker/main.ts` seleciona runner (Pi ou Codex) pelo config serializado | `src/worker/main.ts` | Run containerizado com cada agent completa em teste | done — executor.ts extraído; smoke real docker+codex fake ~1s; 465 testes verdes |
| 3 | `ProjectData.devcontainerPath` + validação + eventos + UI | `src/infrastructure/persistence/project-file-store.ts`, `src/infrastructure/api/routes/projects.ts`, `src/application/orquestrator.ts:518-534`, `web/src/components/projects/*` | CRUD + replay preservam campo; UI edita | done — anti-traversal (../, /, C:); nullable para limpar no update |
| 3 | Build de imagem por projeto: `devcontainer build` com cache por hash; fallback imagem padrão; erro claro p/ compose/runArgs | `src/infrastructure/containers/devcontainer.ts` (novo), integração no supervisor | Projeto com devcontainer.json real gera imagem e o run usa; sem devcontainer usa default; cache hit não rebuilda | done — @devcontainers/cli 0.87; tag kca-devc-<slug>-<hash12>; compose=erro, runArgs=ignorado; imageOverride no supervisor |
