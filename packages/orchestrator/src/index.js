...
import { appendJsonl, boardSnapshot, createTask, getTask, listTasks, moveTask, paths, readCommandResult, readHook, readProject, readSettings, readSettingsScope, recordCommandResult, updateSettings, updateTask, writeTaskFile, writeYaml } from "@kca/fsdb";
...
export async function handleCommand(input, root) {
  ...
  if (command.type === "agent.chat") {
    ...
    await appendJsonl(`${paths(root).runtime}/logs/events.jsonl`, { ts: new Date().toISOString(), type: "agent.event", actor: run.agentId, runId: run.runId, scope: command.scope, taskId: current?.id, prompt: command.prompt, reply, action });
