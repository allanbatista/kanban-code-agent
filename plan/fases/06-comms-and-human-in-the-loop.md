...
- **O que:** Tool de orquestração que escreve a pergunta como **mensagem `assistant` no chat** via o caminho existente `postAgentMessage` (→ `appendChat('assistant','text',pergunta)` + `MESSAGE_APPENDED`), e então **parka o card** em `WAITING(human)` (de F1) e o desidrata (libera o worker) com um **evento de orquestração sem corpo de mensagem** (reusar `TASK_WAITING` com `payload.waitingReason='human'`, ou `TASK_AWAITING_HUMAN` sem texto — o texto já está no chat). Destino (`parent`|`product`|`user`) é **atributo de roteamento** do wait, não um canal separado. (`postAgentMessage` **já é evented** via `MESSAGE_APPENDED`, então a pergunta já notifica o humano por WS.) **Escalonamento recursivo completo (PF9, §9.3):** se o pai não souber (responde “não sei”/escala), repete para o avô e por fim Produto→Usuário, **sem empilhar threads** (cada nível é evento + desidratação), respeitando profundidade. **Caso terminal “ninguém sabe”:** chegando ao topo (Usuário) sem resposta no prazo, cai no timeout→`SUSPENDED` (F6.T3); recusa explícita sem nível acima vira pendência ao Usuário. Idempotente.
...
- **Testes (mock):** `ask_human` → pergunta como `MESSAGE_APPENDED` assistant + status `WAITING(human)`, worker liberado; **escalonamento child→pai→produto→usuário** sem run ativo em nenhum nível; cadeia esgotada sem resposta → timeout/`SUSPENDED`.
- **DoD:** a pergunta é **uma mensagem de chat**; o evento só carrega estado; escalonamento recursivo e caso “ninguém sabe” tratados. `ponytail:` nenhum objeto de pergunta paralelo.
...
- **O que:** O humano responde pelo **endpoint de chat existente** `POST /api/tasks/:taskId/messages` → `appendUserMessage`. **Generalizar `appendUserMessage`** (`orquestrator.ts:521-546`) para, além de reabrir roots em REVIEW/terminal, também **retomar uma task em `WAITING(human)`** — incluindo **subtasks** (remover o guard `!task.options.parentId` para o caso `WAITING(human)`). **Match pergunta↔resposta (PF8, `ponytail`):** **não** criar campo `inReplyTo`/objeto de pergunta — a **primeira mensagem `user` enquanto `status==WAITING(human)` é a resposta** (heurística de estado já suficiente; a pergunta é a última `assistant`). **Timer de deadline (PF3):** o timeout não pode ser reativo (wait groups só reagem a subtasks). Adicionar um **`DeadlineMonitor` simples** — `setInterval` (~30s) no orquestrador que varre tasks `WAITING(human)`/`SUSPENDED` e transiciona para `SUSPENDED` quando `WaitGroup.deadline` (`SWARM_HUMAN_TIMEOUT_MS`) expira; resposta tardia ainda reativa. Manter validação zod em `/messages`.
- **Toca:** `src/infrastructure/api/routes/tasks.ts` (reusa `/messages`, **sem** `/answer`), `orquestrator.ts` (`appendUserMessage` generalizado, `DeadlineMonitor`), `scheduler.ts`.
- **Testes (mock):** 1ª mensagem do usuário em `WAITING(human)` retoma a task (root **e** subtask); deadline expira (relógio fake) → `SUSPENDED` + evento; resposta depois reativa.
- **DoD:** resposta humana é uma mensagem de chat (match por estado, sem campo extra); **nenhum endpoint `/answer`**; timeout disparado por timer proativo, limpo e reversível. `ponytail:` `setInterval` único, sem subsistema de scheduling.
